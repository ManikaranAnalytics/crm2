...
const DashboardPage: React.FC = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
	        if (scope === 'ME' && !user) {
	          // Fallback to ALL if user information is not available.
	          setScope('ALL');
	          setLoading(false);
	          return;
	        }
      ...
      <div className="space-y-8">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
	        ...
	        <section className="grid gap-6 md:grid-cols-2">
	          <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
	            <h3 className="text-sm font-semibold text-slate-900">Team performance (user-wise)</h3>
	            <p className="mt-1 text-xs text-slate-500">
	              Based on the <code className="rounded bg-slate-100 px-1">raised_by</code> field in
	              queries.
	            </p>
	            <div className="mt-4">
	              ...
	              {byUser.length > 0 && (
	                <SimpleBarChart
	                  data={byUser.map((p) => ({
	                    ...p,
	                    label: p.label === 'Advik S' ? 'Himanshu' : p.label,
	                  }))}
