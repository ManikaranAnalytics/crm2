...
interface PssOption {
	...
	useEffect(() => {
		const loadLookups = async () => {
			...
			setError(null);
			try {
				const [clientsRes, pssRes] = await Promise.all([
	...
	const handleClientSelect = (e: React.ChangeEvent<HTMLSelectElement>) => {
		...
		setForm((prev) => ({
			...prev,
			clientId,
			clientName: clients.find((c) => String(c.id) === clientId)?.name || '',
		}));
	};
				...
				<div className="space-y-4">
					...
					>
						...
						<div className="grid gap-4 md:grid-cols-2">
							...
							<div>
								<label className="block text-sm font-medium text-slate-700" htmlFor="clientId">
									Client Name
								</label>
								<select
									id="clientId"
								...
								<p className="mt-1 text-[11px] text-slate-500">
									Manage clients from the Admin section.
								</p>
							</div>
							<div>
