...
const QueriesPage: React.FC = () => {
  ...
  const [updatingId, setUpdatingId] = useState<number | null>(null);
		  ...
		  const [solutionDocAttachment, setSolutionDocAttachment] =
		    useState<MsgAttachmentPayload | null>(null);
  ...
  useEffect(() => {
      ...
      setError(null);
	    	    try {
	    	      const res = await fetch(`/api/queries?scope=my&userId=${user.id}`);
        if (!res.ok) {
    ...
    load();
		  ...
		  const handleStatusChange = async (queryId: number, newStatus: string) => {
		    ...
		    try {
		      ...
		      setQueries((prev) =>
		        prev.map((q) => (q.id === queryId ? { ...q, status: updatedStatus } : q)),
		      );
		    } catch (err: any) {
		  ...
		  const handleSubmitClose = async () => {
		    ...
		    try {
		      ...
		      setQueries((prev) =>
		        prev.map((q) =>
		          q.id === closingQueryId ? { ...q, status: updatedStatus } : q,
		        ),
		      );
		      setClosingQueryId(null);
		      setSolutionAttachment(null);
	    ...
	    return (
	      <Layout>
	        <div className="space-y-4">
	          ...
	          <p className="text-sm text-slate-500">Please sign in to view your queries.</p>
	        </div>
	      </Layout>
	    );
		  }
		  ...
		  const closingQuery =
		    closingQueryId != null
		      ? queries.find((q) => q.id === closingQueryId)
		      : undefined;
	    ...
	    <Layout>
	      <div className="space-y-4">
	        <div>
	          <h2 className="text-2xl font-semibold text-slate-900">Queries</h2>
	          <p className="text-sm text-slate-500">Work with your technical queries.</p>
	        </div>
		        <QueryTabs active="MY" />
		        <div className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
          {error && <p className="border-b border-slate-100 bg-red-50 px-4 py-2 text-sm text-red-700">{error}</p>}
                  ...
                  <th className="px-4 py-2 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">
	                ...
	                {!loading && queries.length === 0 && !error && (
	                  <tr>
		                ...
		                {!loading &&
			                  queries.map((q) => {
	                    const isEditable = EDITABLE_STATUS_OPTIONS.some(
	                      (opt) => opt.value === q.status,
	                    );
	                    return (
					                      <tr key={q.id}>
